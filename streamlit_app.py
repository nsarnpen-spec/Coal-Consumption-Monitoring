...
# ─────────────────────────────────────────────────────────
#  CUSTOM CSS  (dark theme, card styles, alarm banners)
# ─────────────────────────────────────────────────────────
st.markdown("""
<style>
...
# ─────────────────────────────────────────────────────────
#  PARSER  (identical logic to app.py)
# ─────────────────────────────────────────────────────────
...
def process_excel(file_bytes: bytes) -> list[dict]:
    ...
    # Cond.1: ≥2 units > 650 ต่อเนื่อง > 120 นาที
...
def alarm_shapes(data, field, color):
    """สร้าง vrect shapes สำหรับ alarm zones"""
    shapes = []
    in_alarm = False
    start_t  = None
    for r in data:
...
def make_coal_chart(data):
    ...
    fig = go.Figure()
    for key, name, color in [("u1c","Unit 1","#60a5fa"),("u2c","Unit 2","#34d399"),("u3c","Unit 3","#f472b6")]:
    ...
    fig.add_hline(y=650, line_dash="dash", line_color="rgba(249,115,22,0.7)",
                  line_width=1, annotation_text="650 t/h", annotation_font_color="#f97316")
    fig.add_hline(y=670, line_dash="dot",  line_color="rgba(239,68,68,0.85)",
                  line_width=1.5, annotation_text="670 t/h", annotation_font_color="#ef4444")
    fig.update_layout(**DARK_LAYOUT, yaxis=dict(**DARK_LAYOUT["yaxis"], range=[560,730]),
                      shapes=alarm_shapes(data, "a2", "rgba(239,68,68,0.10)"))
    fig.update_yaxes(title_text="t/h", title_font=dict(size=10))
    return fig
...
def make_total_chart(data):
    ...
    fig.add_hline(y=1950, line_dash="dash", line_color="rgba(239,68,68,0.85)",
                  line_width=1.5, annotation_text="1,950 t/h", annotation_font_color="#ef4444")
    fig.update_layout(**DARK_LAYOUT, yaxis=dict(**DARK_LAYOUT["yaxis"], range=[1700,2100]),
                      shapes=alarm_shapes(data, "a4", "rgba(249,115,22,0.12)"))
    fig.update_yaxes(title_text="t/h", title_font=dict(size=10))
    return fig
...
def make_ratio_chart(data):
    ts = [r["t"] for r in data]
    fig = go.Figure()
    for key, name, color in [("u1r","Unit 1","#60a5fa"),("u2r","Unit 2","#34d399"),("u3r","Unit 3","#f472b6")]:
        fig.add_trace(go.Scatter(x=ts, y=[r[key] for r in data], name=name,
                                 line=dict(color=color, width=1.5), mode="lines"))
    fig.add_hline(y=1.08, line_dash="dash", line_color="rgba(239,68,68,0.85)",
                  line_width=1.5, annotation_text="1.08 limit", annotation_font_color="#ef4444")
    fig.update_layout(**DARK_LAYOUT, yaxis=dict(**DARK_LAYOUT["yaxis"], range=[0.95,1.20]),
                      shapes=alarm_shapes(data, "a3", "rgba(234,179,8,0.12)"))
    fig.update_yaxes(title_text="t/MWh", title_font=dict(size=10))
    return fig
...
st.success(f"✅ โหลดสำเร็จ — {len(data):,} records  |  {df['t'].min().strftime('%d/%m/%y %H:%M')} – {df['t'].max().strftime('%d/%m/%y %H:%M')}")

# ─────────────────────────────────────────────────────────
#  TIME RANGE FILTER  (sidebar)
# ─────────────────────────────────────────────────────────
with st.sidebar:
    ...
    if zoom == "6 ชม. แรก":
        ...
        alarm_idx = df.index[df["a2"]].tolist()
        if alarm_idx:
            lo = max(0, alarm_idx[0]-30)
            hi = min(len(df)-1, alarm_idx[-1]+30)
            df = df.iloc[lo:hi+1]
...
#  KPI METRICS
# ─────────────────────────────────────────────────────────
u1c = [r["u1c"] for r in data]; u2c = [r["u2c"] for r in data]; u3c = [r["u3c"] for r in data]
...
cols[3].metric("U1 Peak",      f"{max(u1c):.1f} t/h",  delta="⚠️ >670" if max(u1c)>670 else "✅ ปกติ", delta_color="inverse" if max(u1c)>670 else "off")
cols[4].metric("U2 Peak",      f"{max(u2c):.1f} t/h",  delta="⚠️ >670" if max(u2c)>670 else "✅ ปกติ", delta_color="inverse" if max(u2c)>670 else "off")
cols[5].metric("U3 Peak",      f"{max(u3c):.1f} t/h",  delta="⚠️ >670" if max(u3c)>670 else "✅ ปกติ", delta_color="inverse" if max(u3c)>670 else "off")

cols2 = st.columns(6)
cols2[0].metric("Total Coal Avg",  f"{sum(tots)/len(tots):.0f} t/h")
cols2[1].metric("Total Coal Peak", f"{max(tots):.0f} t/h",   delta="⚠️ >1950" if max(tots)>1950 else "✅ ปกติ", delta_color="inverse" if max(tots)>1950 else "off")
cols2[2].metric("Cond.1 Alarm",    f"{c1} นาที",  delta="ALARM" if c1>0 else "ปกติ", delta_color="inverse" if c1>0 else "off")
cols2[3].metric("Cond.2 Alarm",    f"{c2} นาที",  delta="ALARM" if c2>0 else "ปกติ", delta_color="inverse" if c2>0 else "off")
cols2[4].metric("Ratio >1.08",     f"{c3} นาที",  delta="ALARM" if c3>0 else "ปกติ", delta_color="inverse" if c3>0 else "off")
cols2[5].metric("Total >1950",     f"{c4} นาที",  delta="ALARM" if c4>0 else "ปกติ", delta_color="inverse" if c4>0 else "off")

# ─────────────────────────────────────────────────────────
#  ALARM BANNERS
# ─────────────────────────────────────────────────────────
def banner(cls, icon, text):
    st.markdown(f'<div class="banner banner-{cls}"><span>{icon}</span><span>{text}</span></div>',
                unsafe_allow_html=True)
...
st.markdown('<div class="thr-note"><b style="color:#ef4444">Cond.1:</b> ≥ 2 จาก 3 units &gt; 650 t/h ต่อเนื่อง &gt; 2 ชม. → เปลี่ยน Coal Stockpile &nbsp;|&nbsp; <b style="color:#ef4444">Cond.2:</b> unit ใดเกิน 670 t/h → เตรียม Start Coal Feeder Mill #7</div>', unsafe_allow_html=True)
...
#  ALARM EVENT LIST
# ─────────────────────────────────────────────────────────
st.markdown('<div class="section-h">🚨 Alarm Events – Cond.2 (unit &gt; 670 t/h)</div>', unsafe_allow_html=True)
alarm_rows = [r for r in data if r["a2"]]
if not alarm_rows:
    st.markdown('<p style="color:#6b7280;font-size:0.82rem">ไม่มี Cond.2 Alarm ในช่วงนี้</p>', unsafe_allow_html=True)
else:
    rows_html = ""
    for r in alarm_rows:
        units = []
        ...
        rows_html += f'<div class="alarm-row"><span class="alarm-time">{ts_str}</span><span>{", ".join(units)} t/h</span></div>'
    st.markdown(f'<p style="font-size:0.75rem;color:#8b95a3;margin-bottom:6px">Cond.2: unit เกิน 670 t/h ({len(alarm_rows)} จุด)</p>' + rows_html,
                unsafe_allow_html=True)
...
# ─────────────────────────────────────────────────────────
st.markdown(f"""
<div style="text-align:center;font-size:0.7rem;color:#4b5563;margin-top:24px;padding-bottom:16px;">
  Generated: {datetime.now().strftime('%d/%m/%Y %H:%M')} &nbsp;|&nbsp;
  Source: DCS/PI Minute Snapshot &nbsp;|&nbsp; HONGSA CHS Operation
</div>
""", unsafe_allow_html=True)
